...
function updateCard(elements) {
    // Update Content
    elements.display.quote.textContent = elements.input.quote.value;
    elements.display.insight.textContent = elements.input.insight.value;
    elements.display.name.textContent = elements.input.name.value || "佚名";
    elements.display.cardTitle.textContent = elements.input.cardTitle.value || "卡片标题";
    elements.display.bookTitle.textContent = elements.input.bookTitle.value || "书籍名称";
    elements.display.author.textContent = elements.input.author.value || "作者姓名";

    // Update Date
    if (elements.input.date.value) {
    ...
    elements.display.insight.style.fontFamily = elements.input.textFont.value;
...
function exportToPNG(elements) {
    ...
    html2canvas(card, {
        ...
        elements.output.downloadLink.download = `读者卡片_${new Date().getTime()}.png`;
...
function getElements() {
    return {
        input: {
            ...
            insight: document.getElementById('insight-textarea'),
        ...
        display: {
            ...
            insight: document.getElementById('insight-content'),
...
function setCurrentDate(elements) {
    ...
    elements.display.date.textContent = `${now.getFullYear()}/${now.getMonth() + 1}/${now.getDate()}`;
}
...
function bindEvents(elements) {
    ...
    // Inputs
    elements.input.date.addEventListener('change', () => {
        if (elements.input.date.value) {
            const date = new Date(elements.input.date.value);
            elements.display.date.textContent = `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`;
        }
